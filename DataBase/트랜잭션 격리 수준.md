...
## 1️⃣ READ UNCOMMITTED (레벨0)

다른 트랜잭션에서 커밋되지 않은 내용도 참조할 수 있다.  

![image](https://user-images.githubusercontent.com/63834758/215913267-01881fcd-b24b-4664-b1e9-8dd27e19d3be.png)
...
## 2️⃣ READ COMMITTED (레벨1)

다른 트랜잭션에서 커밋된 내용만 참조할 수 있다.  
실제 테이블 값을 가져오는 것이 아니라 Undo 영역에 백업된 레코드에서 값을 가져온다.

![image](https://user-images.githubusercontent.com/63834758/215914322-e2bd7323-77f7-479c-909a-6bffe5812081.png)
...
- DB2, SQL Server, Sybase : 읽기, 공유 Lock을 이용하여 구현
- Oracle : Lock을 사용하지 않고 쿼리시작 시점의 Undo 데이터를 제공한다.  
...
## 3️⃣ REPEATABLE READ (레벨2)

트랜잭션에 진입하기 이전에 커밋된 내용만 참조할 수 있다.  

![image](https://user-images.githubusercontent.com/63834758/215914455-ebc62555-93cf-405d-b77c-d42d96be1d88.png)
...
- MySQL : 트랜잭션마다 트랜잭션 ID를 부여  → 트랜잭션 ID보다 작은 트랜잭션 번호에서 변경한 것만 읽게 된다.  
( 변경되기 전 레코드는 Undo 공간에 백업해두고 실제 레코드 값을 변경 )



### ✔ 문제점
- 하나의 트랜잭션 실행시간 ↑  → Undo에 백업된 레코드↑ → 멀티 버전을 관리해야하는 단점
- 하지만 영향을 미칠정도로 트랜잭션이 오래 지속되는 경우가 없어서 READ COMMITTED와 REPEATABLE READ의 성능 차이는 거의 없다고 한다.
- UPDATE 부정합, Phantom Read 발생 가능
...
- 변경 후 아직 Commit 되지 않은 값을 읽고, Rollback 후의 값을 다시 읽어 최종 결과 값이 상이한 현상



## 🔸 Non-Repeatable Read (Inconsistent Analysis)

- 한 트랜잭션 내에서 같은 쿼리를 두번 수행할 때, 그 사이에 다른 트랜잭션이 값을 수정 또는 삭제함으로써 두 쿼리가 상이하게 나타나는 비일관성이 발생하는 것
- 다시말해 하나의 트랜잭션 내에서 동일한 SELECT를 수행했을 경우 항상 같은 결과를 반환해야하는 REPEATABLE READ 정합성에 어긋나는 것.

- 금전적 처리와 연결된 서비스에서 문제가 발생할 수 있다.

> 트랜잭션B : 1번 상품의 총 투자액을 조회 👉 100만원이 조회됨  
> 트랜잭션A : 1번 상품의 총 투자액을 120만원으로 바꾸고 COMMIT  
> 트랜잭션B : 1번 상품의 총 투작액을 다시 조회 👉 120만원이 조회됨 (NON-REPEATABLE READ )  
...
- INSERT에 대해서만 발생하는 문제이다. (SELECT, DELETE에 대해서는 발생하지 않는다)
